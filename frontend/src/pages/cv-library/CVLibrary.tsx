...
const getEditorHeight = () =>
  window.innerWidth < 768 ? Math.max(300, window.innerHeight - 220) : 500
...
export function CVLibrary() {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: { name: string; content: string }) =>
      api.post('/cv-library', payload).then((r) => r.data),
    onSuccess: (newItem: CVLibraryItem) => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, ...payload }: { id: string; name: string; content: string }) =>
      api.put(`/cv-library/${id}`, payload).then((r) => r.data),
    onSuccess: (_, { id }) => {
      qc.invalidateQueries({ queryKey: ['cv-library'] })
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.delete(`/cv-library/${id}`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['cv-library'] })
      setSelectedId(null)
  ...
  const startEdit = (item: CVLibraryItem) => {
    setEditingItems((prev) => ({
      ...prev,
